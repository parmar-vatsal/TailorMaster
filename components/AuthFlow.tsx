...
export const AuthFlow: React.FC<AuthFlowProps> = ({ mode, onBack, onSuccess, onSwitchMode }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        if (mode === 'LOGIN') {
            const identifier = cleanData.mobile || cleanData.email;
            if (!identifier) {
            ...
            const result = await db.auth.login(identifier, cleanData.password);
            if (result.success) {
                onSuccess();
            } else {
            ...
            // Register Validation
            if (!cleanData.name || !cleanData.shopName || !cleanData.mobile || !cleanData.password || cleanData.pin.length !== 4) {
                setError('Please fill all required fields correctly. PIN must be 4 digits.');
                setLoading(false);
                return;
            }
            ...
            const result = await db.auth.register({
                ...
                pin: cleanData.pin,
                logo: cleanData.logo || undefined
            });

            if (result.success) {
                // If we get here, registration was successful.
                // Try to auto-login if possible (Trigger handles profile creation now)
                const loginId = cleanData.email || cleanData.mobile;
                const loginResult = await db.auth.login(loginId, cleanData.password);
                
                if (loginResult.success) {
                     onSuccess();
                } else {
                     // If login fails (e.g. email confirmation required), show the message
                     if (loginResult.message && loginResult.message.includes('verify')) {
                         setError('Registration successful! Please check your email to verify your account.');
                     } else {
                         // Switch to login mode manually if auto-login fails for other reasons
                         onSwitchMode();
