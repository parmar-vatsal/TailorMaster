...
export const CustomerList: React.FC = () => {
    ...
    const refreshData = async () => {
        try {
            const [c, o, m] = await Promise.all([db.customers.getAll(), db.orders.getAll(), db.measurements.getAll()]);
            setCustomers(c);
            setAllOrders(o);
                                                ...
                                                <div key={order.id} className="bg-white rounded-2xl border border-slate-200 p-5 flex justify-between items-center shadow-sm hover:shadow-md transition-all">
                                                    <div>
                                                        <div className="flex items-center gap-3 mb-2">
                                                            <span className="font-bold text-slate-900 text-lg">{order.garmentType}</span>
