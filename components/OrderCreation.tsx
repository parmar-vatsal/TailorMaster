...
export const OrderCreation: React.FC<OrderCreationProps> = ({ onCancel, onSuccess }) => {
  ...
  useEffect(() => {
    const loadMeasurements = async () => {
      if (customer) {
        ...
        await Promise.all(Object.values(GarmentType).map(async (type) => {
          const existing = await db.measurements.getByCustomerAndType(customer.id, type);
          if (existing) {
            loadedData[type] = { ...existing.data };
          }
        }));
  ...
  const handleCustomerSearch = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setMobileSearch(val);
    if (val.length === 10) {
      const found = await db.customers.getByMobile(val);
      if (found) {
        setCustomer(found);
    ...
    try {
        const savePromises = Object.values(GarmentType).map(async (gType) => {
            ...
            if (hasData) {
                ...
                return db.measurements.save(measurement);
            }
        });

        await Promise.all(savePromises);

        const garmentTypeString = selectedItems.join(', ');
        const primaryMeasurementId = `${customer.id}_${selectedItems[0]}`;
                ...
                {[1, 2, 3].map((i) => {
                    ...
                    return (
                        <div key={i} className={`flex flex-col items-center gap-2`}>
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold border-4 transition-all duration-300 z-10 bg-white ${
                                isActive 
                        ...
                        <div className="grid grid-cols-4 gap-2 mb-8 bg-white p-2 rounded-2xl border border-slate-200 shadow-sm">
                            {Object.values(GarmentType).map((type) => (
                                <button
                                    ...
                                    className={`py-3 px-1 rounded-xl text-xs sm:text-sm font-bold text-center border transition-all duration-200 ${
                                    currentTab === type
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    {Object.values(GarmentType).map(type => (
                                        <button
                                            ...
                                            className={`p-4 rounded-2xl border-2 font-bold text-sm transition-all flex items-center justify-between ${
