...
export const InvoiceView: React.FC<InvoiceViewProps> = ({ orderId, onBack }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
        try {
            const o = await db.orders.getById(orderId);
            if (o) {
                // Parse "Shirt, Pant" into array
                const types = o.garmentType.split(',').map(t => t.trim()) as GarmentType[];
                
                const [c, conf] = await Promise.all([
                    db.customers.getById(o.customerId),
                    db.config.get()
                ]);

                const measList: Measurement[] = [];
                await Promise.all(types.map(async (t) => {
                    const m = await db.measurements.getByCustomerAndType(o.customerId, t);
                    if (m) measList.push(m);
                }));
                        ...
                        <div className="flex-1">
                             ...
                             <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                <div className="flex justify-between items-center mb-2">
                                    <span className="font-bold text-slate-800 text-lg">{order.garmentType}</span>
                                ...
                                {measurements.map((m) => (
                                    <div key={m.id} className="bg-white border border-slate-200 rounded-xl p-0 overflow-hidden">
                                        ...
                                        <div className="p-4 flex flex-wrap gap-2">
                                            {Object.entries(m.data).map(([key, val]) => val && (
                                                <div key={key} className="border border-slate-100 rounded-lg px-3 py-1.5 flex flex-col items-center min-w-[60px] bg-white">
                                                    ...
                                                    <span className="text-sm font-bold text-slate-900 leading-none">{val}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-end gap-6">
                        ...
                        <div className="w-full md:w-auto min-w-[240px]">
                            ...
                            <div className="border-t-2 border-dashed border-slate-200 pt-3 flex justify-between items-center relative">
                                ...
                                <div className={`border-2 rounded-lg px-3 py-1 flex items-center justify-center transform -rotate-6 ${
                                    isPaid 
